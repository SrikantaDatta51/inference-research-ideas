...
\begin{tikzpicture}[
    ...
    level/.style={draw, rounded corners=4pt, minimum height=0.65cm, font=\scriptsize\sffamily, thick, text=black},
    comp/.style={draw, rounded corners=2pt, minimum height=0.45cm, font=\tiny\sffamily, thick, text=black},
    engine/.style={draw, rounded corners=5pt, minimum height=0.8cm, font=\scriptsize\sffamily\bfseries, thick, text=black},
    output/.style={draw, rounded corners=3pt, minimum height=0.55cm, font=\tiny\sffamily, thick, text=black},
    arr/.style={-{Stealth[length=2mm]}, thick, black},
    darr/.style={-{Stealth[length=2mm]}, thick, black, dashed},
    brace/.style={decorate, decoration={brace, amplitude=4pt, mirror}, thick, black},
]
% ── LEVEL 1: NODE TWINS (bottom of hierarchy) ──
\node[level, fill=blue!8, minimum width=2.6cm] (n1) at (0,0) {Node Twin 1};
\node[level, fill=blue!8, minimum width=2.6cm, right=0.3cm of n1] (n2) {Node Twin 2};
\node[level, fill=blue!8, minimum width=1.2cm, right=0.3cm of n2] (ndots) {$\cdots$};
\node[level, fill=blue!8, minimum width=2.6cm, right=0.3cm of ndots] (nn) {Node Twin $N$};

% Node detail boxes (inside Node Twin 1)
\node[comp, fill=blue!4, below=0.15cm of n1, xshift=-0.7cm, minimum width=1.0cm] (gpu1) {\tiny 8$\times$GPU};
\node[comp, fill=blue!4, below=0.15cm of n1, xshift=0.7cm, minimum width=1.0cm] (kv1) {\tiny KV-Cache};

% ── LEVEL 2: RACK TWINS ──
\node[level, fill=green!10, minimum width=5.2cm, above=1.4cm of n1, xshift=1.5cm] (r1) {Rack Twin 1 (Thermal + ToR + Airflow)};
\node[level, fill=green!10, minimum width=3.8cm, above=1.4cm of nn, xshift=-0.3cm] (r2) {Rack Twin $R$};

% ── LEVEL 3: DC TWIN ──
\node[level, fill=orange!10, minimum width=7cm, above=0.9cm of r1, xshift=2.2cm] (dc) {Datacenter Twin (Cooling Loops + Power Distribution + CRAH)};

% ── LEVEL 4: FLEET TWIN ──
\node[level, fill=red!8, minimum width=7cm, above=0.7cm of dc] (fleet) {Fleet Twin (Cross-DC Routing + Workload Migration + Carbon Opt.)};

% Level labels (left side)
\node[font=\tiny\sffamily\bfseries, text=blue!70, left=0.2cm of n1] {L1};
\node[font=\tiny\sffamily\bfseries, text=green!50!black, left=0.2cm of r1] {L2};
\node[font=\tiny\sffamily\bfseries, text=orange!70!black, left=0.2cm of dc] {L3};
\node[font=\tiny\sffamily\bfseries, text=red!70!black, left=0.2cm of fleet] {L4};

% Timescale labels (right side)
\node[font=\tiny\itshape, text=black!60, right=0.15cm of nn] {ms--s};
\node[font=\tiny\itshape, text=black!60, right=0.15cm of r2] {s--min};
\node[font=\tiny\itshape, text=black!60] at (dc.east -| r2.east) [right=0.15cm] {min--hr};
\node[font=\tiny\itshape, text=black!60] at (fleet.east -| r2.east) [right=0.15cm] {hr--day};

% ── Upward aggregation arrows ──
\draw[arr] (n1.north) -- (r1.south -| n1.north);
\draw[arr] (n2.north) -- (r1.south -| n2.north);
\draw[arr] (nn.north) -- (r2.south);
\draw[arr] (r1.north) -- (dc.south -| r1.north);
\draw[arr] (r2.north) -- (dc.south -| r2.north);
\draw[arr] (dc.north) -- (fleet.south);

% ── TELEMETRY INPUT (below Level 1) ──
\node[level, fill=gray!8, minimum width=11.8cm, below=1.0cm of ndots] (telem) {Production Telemetry: DCGM (47 metrics/GPU) + vLLM (23 metrics/inst) + Network (12 metrics/port)};
\draw[arr] (telem.north -| n1.south) -- (n1.south);
\draw[arr] (telem.north -| n2.south) -- (n2.south);
\draw[arr] (telem.north -| nn.south) -- (nn.south);

% ── NEURAL SDE ENGINE (right side) ──
\node[engine, fill=purple!10, minimum width=3.2cm, minimum height=1.6cm] (sde) at (14.5,2.2)
  {\begin{tabular}{c}\textbf{Neural SDE Engine}\\[2pt]$dz = f_\theta(z,a)\,dt$\\$+ g_\phi(z)\,dW_t$\end{tabular}};

% ── OUTPUT MODULES (around SDE) ──
\node[output, fill=yellow!12, minimum width=2.8cm, above=0.6cm of sde] (causal) {Causal Counterfactual};
\node[output, fill=cyan!10, minimum width=2.8cm, below=0.6cm of sde] (dreamer) {DreamerV3-Infra (RL)};
\node[output, fill=pink!10, minimum width=2.8cm, above=0.6cm of causal] (anomaly) {Anomaly Detection};
\node[output, fill=lime!10, minimum width=2.8cm, below=0.6cm of dreamer] (foundation) {Foundation Model};

% ── Connect hierarchy to SDE ──
\draw[darr] (dc.east) -- ++(1.2,0) |- (sde.west);
\draw[darr] (fleet.east) -- ++(0.8,0) |- (sde.north west);

% ── SDE to outputs ──
\draw[arr] (sde.north) -- (causal.south);
\draw[arr] (sde.south) -- (dreamer.north);
\draw[arr] (causal.north) -- (anomaly.south);
\draw[arr] (dreamer.south) -- (foundation.north);

% ── Feedback from RL to hierarchy ──
\draw[arr, densely dotted] (dreamer.west) -| ($(r1.north east) + (0.3,0.3)$) -- (dc.south -| r1.north east);

% ── Annotations ──
\node[font=\tiny\itshape, text=black!50, below=0.05cm of causal] {do-calculus, $<$50ms};
\node[font=\tiny\itshape, text=black!50, above=0.05cm of dreamer] {1000$\times$ real-time};
\node[font=\tiny\itshape, text=black!50, below=0.05cm of anomaly] {12--47 min early warning};
\node[font=\tiny\itshape, text=black!50, above=0.05cm of foundation] {$<$1\% data transfer};

% ── Brace for hierarchy ──
\draw[brace] (-1.3, -0.4) -- (-1.3, 4.8) node[midway, left=0.15cm, font=\tiny\sffamily, text=black, rotate=90, anchor=south] {Hierarchical Digital Twin};
\end{tikzpicture}
\caption{InferTwin system architecture. \textbf{Left}: Four-level hierarchical digital twin mirrors datacenter physical structure---each level operates at its natural timescale, with state aggregating upward and constraints propagating downward. \textbf{Right}: Neural SDE engine powers four applications: anomaly detection (12--47 min early warning), causal counterfactual queries ($<$50 ms), DreamerV3-Infra offline RL (1000$\times$ real-time training), and a foundation model for cross-facility transfer.}
\label{fig:arch}
\end{figure*}
...
\begin{equation}
\text{BW}_{\text{avail}}(i,j) = \frac{\text{BW}_{\max}}{1 + \textstyle\sum_{(k,l) \neq (i,j)} c_{ijkl}}
\end{equation}
...
\begin{equation}
P(\text{tput} \mid do(\text{pcap}{=}500)) = \textstyle\sum_{z} P(\text{tput} \mid z) P(z \mid do(\text{pcap}))
\end{equation}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Days} & \textbf{1hr} & \textbf{24hr} & \textbf{F1} & \textbf{Note} \\
\midrule
