...
\centering
\begin{tikzpicture}[
    lev/.style={draw, rounded corners=4pt, minimum height=0.65cm, font=\scriptsize\sffamily\bfseries, line width=0.6pt, text=black},
    sub/.style={font=\tiny\sffamily, text=black!55},
    eng/.style={draw, rounded corners=5pt, minimum height=1.2cm, font=\small\sffamily\bfseries, line width=0.8pt, text=black},
    app/.style={draw, rounded corners=3pt, minimum height=0.6cm, font=\scriptsize\sffamily, line width=0.5pt, text=black},
    tel/.style={draw, rounded corners=3pt, minimum height=0.55cm, font=\scriptsize\sffamily, line width=0.5pt, text=black},
    arr/.style={-{Stealth[length=2mm]}, line width=0.6pt, black},
    darr/.style={-{Stealth[length=2mm]}, line width=0.5pt, black, dashed},
    feed/.style={-{Stealth[length=1.5mm]}, line width=0.4pt, black, densely dotted},
]
% ═══════ HIERARCHY (center, 4 rows) ═══════

% L4 Fleet Twin
\node[lev, fill=red!15, minimum width=9.5cm] (fleet) at (4,4.4) {Fleet Twin};
\node[sub, anchor=west] at (fleet.east) [right=0.15cm] {hr--day};
\node[sub] at (4,4.0) {Cross-DC Routing \textbullet{} Workload Migration \textbullet{} Carbon-Aware Scheduling};

% L3 Datacenter Twin
\node[lev, fill=orange!18, minimum width=9.5cm] (dc) at (4,3.1) {Datacenter Twin};
\node[sub, anchor=west] at (dc.east) [right=0.15cm] {min--hr};
\node[sub] at (4,2.7) {CRAH Units \textbullet{} Power Distribution \textbullet{} Cooling Loops \textbullet{} Network Fabric};

% L2 Rack Twins
\node[lev, fill=green!20, minimum width=4.4cm] (r1) at (1.8,1.8) {Rack Twin 1};
\node[lev, fill=green!20, minimum width=4.4cm] (r2) at (6.2,1.8) {Rack Twin $R$};
\node[sub, anchor=west] at (r2.east) [right=0.15cm] {s--min};
\node[sub] at (1.8,1.4) {Thermal + ToR + Airflow};
\node[sub] at (6.2,1.4) {Power + Cooling};

% L1 Node Twins
\node[lev, fill=blue!18, minimum width=2.0cm] (n1) at (0.5,0.5) {Node 1};
\node[lev, fill=blue!18, minimum width=2.0cm] (n2) at (2.8,0.5) {Node 2};
\node[lev, fill=blue!15, minimum width=0.6cm] (nd) at (4.2,0.5) {$\cdots$};
\node[lev, fill=blue!18, minimum width=2.0cm] (nn) at (5.6,0.5) {Node $N_{r1}$};
\node[lev, fill=blue!15, minimum width=0.6cm] (nd2) at (7.0,0.5) {$\cdots$};
\node[lev, fill=blue!18, minimum width=1.5cm] (nnr) at (8.0,0.5) {Node $N$};
\node[sub, anchor=west] at (nnr.east) [right=0.15cm] {ms--s};

% L1 sub-components
\node[sub] at (0.5,0.05) {8$\times$B200 + KV};
\node[sub] at (2.8,0.05) {8$\times$B200 + KV};

% Level labels
\node[font=\scriptsize\sffamily\bfseries, text=red!60] at (-1.0,4.4) {L4};
\node[font=\scriptsize\sffamily\bfseries, text=orange!60] at (-1.0,3.1) {L3};
\node[font=\scriptsize\sffamily\bfseries, text=green!50!black] at (-1.0,1.8) {L2};
\node[font=\scriptsize\sffamily\bfseries, text=blue!60] at (-1.0,0.5) {L1};

% Upward arrows
\draw[arr] (n1.north) -- ++(0,0.45);
\draw[arr] (n2.north) -- ++(0,0.45);
\draw[arr] (nn.north) -- ++(0,0.45);
\draw[arr] (r1.north) -- ++(0,0.45);
\draw[arr] (r2.north) -- ++(0,0.45);
\draw[arr] (dc.north) -- (fleet.south);

% ═══════ TELEMETRY (bottom) ═══════
\node[tel, fill=gray!12, minimum width=9.5cm] (telem) at (4,-0.6)
  {Telemetry: DCGM (47 met/GPU) + vLLM (23 met/inst) + Network (12 met/port)};
\draw[arr] (telem.north -| n1) -- (n1.south);
\draw[arr] (telem.north -| n2) -- (n2.south);
\draw[arr] (telem.north -| nn) -- (nn.south);
\draw[arr] (telem.north -| nnr) -- (nnr.south);

% ═══════ NEURAL SDE ENGINE (right side) ═══════
\node[eng, fill=purple!15, minimum width=4.5cm, minimum height=2.0cm] (sde) at (13.2,2.5)
  {\begin{tabular}{c}Neural SDE Engine\\[3pt]$dz = f_\theta(z,a)\,dt + g_\phi(z)\,dW_t$\\[2pt]{\scriptsize $d_z{=}128$ latent, $d_s{=}176$ state}\end{tabular}};

% ═══════ 4 APPLICATION MODULES ═══════
\node[app, fill=pink!18, minimum width=3.8cm] (anom) at (13.2,4.8) {Anomaly Detection};
\node[sub, right=0.1cm of anom, anchor=west] {12--47 min early};

\node[app, fill=yellow!18, minimum width=3.8cm] (caus) at (13.2,4.1) {Causal Counterfactual};
\node[sub, right=0.1cm of caus, anchor=west] {do-calculus, $<$50\,ms};

\node[app, fill=cyan!15, minimum width=3.8cm] (drmr) at (13.2,0.9) {DreamerV3-Infra (RL)};
\node[sub, right=0.1cm of drmr, anchor=west] {1000$\times$ real-time};

\node[app, fill=lime!15, minimum width=3.8cm] (fmod) at (13.2,0.2) {Foundation Model};
\node[sub, right=0.1cm of fmod, anchor=west] {$<$1\% data transfer};

% SDE to applications
\draw[arr] (sde.north) -- ++(0,0.3) -- (caus.south);
\draw[arr] (caus.north) -- (anom.south);
\draw[arr] (sde.south) -- ++(0,-0.3) -- (drmr.north);
\draw[arr] (drmr.south) -- (fmod.north);

% Hierarchy → SDE
\draw[darr, line width=0.7pt] (dc.east) -- ++(0.5,0) |- (sde.west);
\draw[darr, line width=0.7pt] (fleet.east) -- ++(0.3,0) |- ([yshift=0.3cm]sde.west);

% Feedback: RL → hierarchy
\draw[feed] (drmr.west) -- ++(-1.0,0) |- ([yshift=-0.15cm]dc.east);

% Downward constraint arrow (side annotation)
\draw[arr, red!50, line width=0.4pt] ([xshift=-0.3cm]fleet.south) -- ++(0,-0.3);
\draw[arr, red!50, line width=0.4pt] ([xshift=-0.3cm]dc.south) -- ++(0,-0.3);
\node[font=\tiny\itshape, text=red!40, rotate=90, anchor=south] at (-0.6,3.7) {constraints $\downarrow$};
\node[font=\tiny\itshape, text=blue!40, rotate=90, anchor=north] at (-0.3,3.7) {state $\uparrow$};
\end{tikzpicture}
\caption{InferTwin system architecture. \textbf{Left}: Four-level hierarchical digital twin (L1--L4) mirrors the physical datacenter---state aggregates upward while constraints propagate downward at each level's natural timescale. \textbf{Right}: Neural SDE engine ($d_z{=}128$ latent, $d_s{=}176$ state) powers four applications: anomaly detection (12--47\,min early warning), causal counterfactual queries ($<$50\,ms), DreamerV3-Infra offline RL (1000$\times$ real-time training), and cross-facility foundation model transfer.}
\label{fig:arch}
\end{figure*}
