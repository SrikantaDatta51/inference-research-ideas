...
\draw[arr] (gpu1.east) -- (sw.west |- gpu1); \draw[arr] (gpu2.east) -- (sw);
\draw[arr] (dpu) -- (ctrl);
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    arr/.style={-{Stealth[length=1.5mm]}, line width=0.4pt, black},
]
\node[box, fill=yellow!20, minimum width=6cm] (solar) at (0,0) {Solar Array + BESS + Inverter};
\node[box, fill=blue!15, minimum width=6cm] (compute) at (0,0.9) {AMD MI355X $\times$16 + EPYC CPUs};
\node[box, fill=green!15, minimum width=6cm] (network) at (0,1.7) {Pensando DPU + Arista Switch + RoCE};
\node[box, fill=purple!15, minimum width=6cm] (ctrl) at (0,2.5) {Zetta Control Plane (Papers 1--8)};
\node[box, fill=red!12, minimum width=6cm] (fleet) at (0,3.3) {Fleet Management + RL Routing};
\draw[arr] (solar) -- (compute); \draw[arr] (compute) -- (network);
\draw[arr] (network) -- (ctrl); \draw[arr] (ctrl) -- (fleet);
\end{tikzpicture}
\caption{SolarInfer appliance software stack.}
\label{fig:stack}
\end{figure}
...
\emph{Import duties and VAT.} GPU accelerators imported into the EU face 14\% import duty + 19--25\% VAT. Solar panels are VAT-exempt in most EU countries under the Energy Directive. Net impact: approximately 8--12\% increase in total CAPEX for EU deployments, partially offset by EU-specific incentives (German EEG surcharge exemption, French solar tax credit).
