...
\begin{tikzpicture}[
    ...
    darr/.style={-{Stealth[length=2mm]}, line width=0.4pt, black, dashed, shorten >=4pt, shorten <=4pt},
    region/.style={draw, dashed, rounded corners=6pt, black!30, line width=0.4pt, inner sep=8pt},
]
% --- Column 1: Infrastructure Layers ---
\node[box, fill=blue!12, minimum width=2cm] (nic) at (0,0) {L1: NIC};
\node[box, fill=blue!16, minimum width=2cm] (link) at (2.4,0) {L2: Link};
\node[box, fill=green!12, minimum width=2cm] (gpu) at (4.8,0) {L3: GPU};
\node[box, fill=green!16, minimum width=2cm] (inter) at (7.2,0) {L4: Interconnect};
\node[box, fill=orange!12, minimum width=2cm] (intra) at (0,1.3) {L5: Intra-Node};
\node[box, fill=orange!16, minimum width=2cm] (rack) at (2.4,1.3) {L6: Rack};
\node[box, fill=red!10, minimum width=2cm] (dc) at (4.8,1.3) {L7: DC};
\node[box, fill=red!14, minimum width=2cm] (cloud) at (7.2,1.3) {L8: Multi-Cloud};
\node[region, fit=(nic)(link)(gpu)(inter)(intra)(rack)(dc)(cloud), label={[font=\tiny\sffamily\bfseries, text=black!50]above:Infrastructure Layers (47 failure modes)}] {};

% --- Column 2: Signal Pipeline ---
\node[box, fill=purple!12, minimum width=3cm, minimum height=1.4cm] (signals) at (3.6,3.6) {\begin{tabular}{c}\scriptsize Signal Pipeline\\\tiny 312 metrics @ 100ms\\\tiny (Pensando DPU)\end{tabular}};

% --- Column 3: RL Agents ---
\node[box, fill=cyan!10, minimum width=2.6cm] (ppo) at (10.2,0.3) {Node PPO};
\node[box, fill=cyan!14, minimum width=2.6cm] (sac) at (10.2,1.3) {Rack SAC};
\node[box, fill=cyan!18, minimum width=2.6cm] (cql) at (10.2,2.3) {DC CQL};
\node[font=\tiny\sffamily, text=black!45] at (10.2,-0.15) {100ms cycle};
\node[font=\tiny\sffamily, text=black!45] at (10.2,0.85) {1s cycle};
\node[font=\tiny\sffamily, text=black!45] at (10.2,1.85) {10s cycle};
\node[region, fit=(ppo)(sac)(cql), label={[font=\tiny\sffamily\bfseries, text=black!50]above:Hierarchical RL Agents}] {};

% --- Column 4: Actions ---
\node[box, fill=yellow!12, minimum width=2.2cm] (migrate) at (14,0.3) {CRIU Migrate};
\node[box, fill=yellow!16, minimum width=2.2cm] (drain) at (14,1.3) {Drain / Re-shard};
\node[box, fill=yellow!10, minimum width=2.2cm] (xcloud) at (14,2.3) {Cross-Cloud};
\node[region, fit=(migrate)(drain)(xcloud), label={[font=\tiny\sffamily\bfseries, text=black!50]above:Actions}] {};

% --- Arrow: Infra region -> Signal (from above the region box) ---
\draw[-{Stealth[length=2mm]}, line width=0.5pt, black!50] (3.6,2.5) -- (3.6,2.8);

% --- Arrows: Signal -> RL Agents (horizontal then vertical, no touching) ---
\draw[-{Stealth[length=2mm]}, line width=0.5pt, black] (5.2,3.6) -- (7.5,3.6) -- (7.5,0.3) -- (8.8,0.3);
\draw[-{Stealth[length=2mm]}, line width=0.5pt, black] (7.5,1.3) -- (8.8,1.3);
\draw[-{Stealth[length=2mm]}, line width=0.5pt, black] (7.5,2.3) -- (8.8,2.3);

% --- Arrows: RL -> Actions (clean horizontal) ---
\draw[-{Stealth[length=2mm]}, line width=0.5pt, black] (11.6,0.3) -- (12.8,0.3);
\draw[-{Stealth[length=2mm]}, line width=0.5pt, black] (11.6,1.3) -- (12.8,1.3);
\draw[-{Stealth[length=2mm]}, line width=0.5pt, black] (11.6,2.3) -- (12.8,2.3);

% --- Feedback: below everything, no overlap ---
\draw[-{Stealth[length=2mm]}, line width=0.4pt, black, dashed] (14,-0.2) -- (14,-1.0) -- (-2,-1.0) -- (-2,3.6) -- (2,3.6);
\node[font=\tiny\sffamily\itshape, text=black!40] at (6,-1.2) {Outcome feedback to RL training};
\end{tikzpicture}%
}
\caption{ReliableInfer end-to-end architecture. Eight infrastructure layers (left) feed 312 metrics into the DPU-based signal pipeline (center-left). Three hierarchical RL agents (center-right) produce actions (right), with migration outcomes feeding back into training.}
\label{fig:arch}
\end{figure*}
...
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[
    layer/.style={draw, rounded corners=2pt, minimum height=0.45cm, font=\tiny\sffamily\bfseries, text=black, line width=0.4pt},
    arr/.style={-{Stealth[length=1.2mm]}, line width=0.3pt, black!50, shorten >=3pt, shorten <=3pt},
]
% Layers stacked
\node[layer, fill=red!20, minimum width=6.5cm] (mc) at (0,3.5) {L8: Multi-Cloud (3 modes)};
\node[layer, fill=red!15, minimum width=6cm] (dc) at (0,3.0) {L7: Datacenter (5 modes)};
\node[layer, fill=orange!20, minimum width=5.5cm] (rk) at (0,2.5) {L6: Rack (6 modes)};
\node[layer, fill=orange!15, minimum width=5cm] (in) at (0,2.0) {L5: Intra-Node (8 modes)};
\node[layer, fill=green!20, minimum width=4.5cm] (ix) at (0,1.5) {L4: Interconnect (5 modes)};
\node[layer, fill=green!15, minimum width=4cm] (gp) at (0,1.0) {L3: GPU/Accelerator (6 modes)};
\node[layer, fill=blue!20, minimum width=3.5cm] (lk) at (0,0.5) {L2: Link/Fabric (8 modes)};
\node[layer, fill=blue!15, minimum width=3cm] (nc) at (0,0.0) {L1: NIC (6 modes)};

% Blast radius annotations
\node[font=\tiny\sffamily, text=black!50] at (4.2,0) {1 port};
\node[font=\tiny\sffamily, text=black!50] at (4.2,1.0) {1 GPU};
\node[font=\tiny\sffamily, text=black!50] at (4.2,2.0) {1 node};
\node[font=\tiny\sffamily, text=black!50] at (4.2,2.5) {16--48 nodes};
\node[font=\tiny\sffamily, text=black!50] at (4.2,3.5) {Region};

% Escalation arrow
\draw[arr, line width=0.6pt] (-3.8,0) -- (-3.8,3.5);
\node[font=\tiny\sffamily\bfseries, text=black!60, rotate=90] at (-4.2,1.75) {Escalation};

% MTTR arrow
\draw[arr, line width=0.6pt] (5.2,0) -- (5.2,3.5);
\node[font=\tiny\sffamily\bfseries, text=black!60, rotate=90] at (5.6,1.75) {MTTR increases};
\end{tikzpicture}%
}
\caption{Seven-layer failure taxonomy. Lower layers have smaller blast radius and faster recovery. Failures escalate upward when unresolved.}
\label{fig:taxonomy}
\end{figure}
...
\begin{tikzpicture}[
    ...
    arr/.style={-{Stealth[length=2mm]}, thick, black, shorten >=4pt, shorten <=4pt},
    darr/.style={-{Stealth[length=2mm]}, thick, black, dashed, shorten >=4pt, shorten <=4pt},
]
% Tiers
...
\draw[arr] (t1b) -- (t2);
\draw[arr] (t2) -- (t3);
\draw[darr] (t3.west) -- ++(-0.6,0) |- (t1a.west);
\draw[darr] (t3.east) -- ++(0.6,0) |- (t1b.east);

% Labels
\node[font=\tiny, text=black!60] at (-2.8,2.2) {escalate};
\node[font=\tiny, text=black!60] at (2.8,2.2) {constrain};
\end{tikzpicture}
...
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[
    phase/.style={draw, rounded corners=3pt, minimum width=1.6cm, minimum height=0.55cm, font=\tiny\sffamily\bfseries, text=black, line width=0.5pt},
    arr/.style={-{Stealth[length=1.5mm]}, line width=0.5pt, black, shorten >=3pt, shorten <=3pt},
    lbl/.style={font=\tiny\sffamily, text=black!60},
]
% Source node
\node[phase, fill=blue!15] (pre) at (0,0) {Pre-Stage};
\node[phase, fill=blue!20] (chk) at (2.2,0) {Checkpoint};
\node[phase, fill=orange!18] (xfer) at (4.4,0) {Transfer};
\node[phase, fill=green!18] (rest) at (6.6,0) {Restore};

% Timing labels
\node[lbl] at (0,-0.5) {30--120s ahead};
\node[lbl] at (2.2,-0.5) {280ms};
\node[lbl] at (4.4,-0.5) {320ms};
\node[lbl] at (6.6,-0.5) {480ms};

% Sub-items
\node[font=\tiny\sffamily, text=black!50, align=center] at (0,0.55) {Model weights\\via RDMA};
\node[font=\tiny\sffamily, text=black!50, align=center] at (2.2,0.55) {CPU + GPU\\+ KV-cache};
\node[font=\tiny\sffamily, text=black!50, align=center] at (4.4,0.55) {RoCE v2\\25--100 Gbps};
\node[font=\tiny\sffamily, text=black!50, align=center] at (6.6,0.55) {Reconstruct\\+ warm cache};
...
% Timeline bar
\draw[line width=1.5pt, green!60!black] (0,-0.9) -- (1.8,-0.9);
\draw[line width=1.5pt, red!60] (1.8,-0.9) -- (6.6,-0.9);
\draw[line width=1.5pt, green!60!black] (6.6,-0.9) -- (7.8,-0.9);
\node[font=\tiny\sffamily, text=green!60!black] at (0.9,-1.15) {Serving};
\node[font=\tiny\sffamily, text=red!60] at (4.2,-1.15) {Downtime: 1.3s};
\node[font=\tiny\sffamily, text=green!60!black] at (7.2,-1.15) {Serving};
\end{tikzpicture}%
}
\caption{CRIU migration pipeline. Pre-staging overlaps with active serving; user-visible downtime is only 1.3\,s for intra-vendor migration.}
\label{fig:criu}
\end{figure}
...
\centering
\begin{tikzpicture}[
    box/.style={draw, rounded corners=3pt, minimum width=1.4cm, minimum height=0.5cm, font=\tiny\sffamily\bfseries, text=black, line width=0.5pt},
    arr/.style={-{Stealth[length=1.5mm]}, line width=0.5pt, black, shorten >=3pt, shorten <=3pt},
]
% Pipeline stages
\node[box, fill=blue!12] (raw) at (0,0) {Raw Metrics};
\node[box, fill=blue!18] (norm) at (0,0.9) {Normalize};
\node[box, fill=orange!15] (anom) at (0,1.8) {Anomaly Score};
\node[box, fill=orange!20] (corr) at (0,2.7) {Correlation};
\node[box, fill=green!15] (agg) at (0,3.6) {Aggregate};

% RL agents
\node[box, fill=purple!15] (ppo) at (2.5,1.8) {PPO Agent};
\node[box, fill=purple!20] (sac) at (2.5,2.7) {SAC Agent};
\node[box, fill=purple!25] (cql) at (2.5,3.6) {CQL Agent};

% LSTM predictor
\node[box, fill=red!12] (lstm) at (-2.2,2.7) {LSTM Predictor};

% Labels
\node[font=\tiny\sffamily, text=black!50] at (-1.2,0) {312/node};
\node[font=\tiny\sffamily, text=black!50] at (-1.2,0.9) {[0,1] range};
\node[font=\tiny\sffamily, text=black!50] at (-1.2,1.8) {EWMA};
\node[font=\tiny\sffamily, text=black!50] at (-1.5,2.7) {47$\times$47};
...
\draw[arr] (anom) -- (corr);
\draw[arr] (corr) -- (agg);
\draw[arr] (agg.east) -- (cql.west);
\draw[arr] (corr.east) -- (sac.west);
\draw[arr] (anom.east) -- (ppo.west);
\draw[arr] (corr.west) -- (lstm.east);
\draw[arr] (lstm.south) |- (ppo.west);

% DPU label
\node[font=\tiny\sffamily\bfseries, text=black!60] at (-1.1,-0.5) {AMD Pensando DPU};
\draw[dashed, rounded corners=5pt, black!40] (-2.8,-0.7) rectangle (0.8,3.2);
\end{tikzpicture}
\caption{Signal processing pipeline on DPU. Raw metrics flow through four stages before reaching RL agents. LSTM predictor runs on DPU ARM cores.}
\label{fig:signal}
\end{figure}
...
\resizebox{\columnwidth}{!}{%
\begin{tikzpicture}[
    decision/.style={draw, diamond, aspect=2, minimum width=1.2cm, minimum height=0.6cm, font=\tiny\sffamily\bfseries, text=black, line width=0.5pt, inner sep=1pt},
    action/.style={draw, rounded corners=3pt, minimum width=1.5cm, minimum height=0.4cm, font=\tiny\sffamily\bfseries, text=black, line width=0.5pt},
    arr/.style={-{Stealth[length=1.5mm]}, line width=0.5pt, black, shorten >=3pt, shorten <=3pt},
    lbl/.style={font=\tiny\sffamily, text=black!60},
]
\node[action, fill=blue!15] (detect) at (0,3.5) {Failure Detected};
\node[decision, fill=yellow!20] (sev) at (0,2.5) {Severity?};
\node[action, fill=red!15] (auto) at (-2.5,1.5) {Auto Execute};
\node[decision, fill=yellow!15] (cost) at (0,1.5) {Cost OK?};
\node[action, fill=orange!15] (semi) at (0,0.5) {Semi-Auto};
\node[action, fill=green!15] (manual) at (2.5,1.5) {Manual Only};

\draw[arr] (detect) -- (sev);
\draw[arr] (sev) -- node[lbl, left] {Critical} (auto);
\draw[arr] (sev) -- node[lbl, right] {High cost} (manual);
\draw[arr] (sev) -- (cost);
\draw[arr] (cost) -- node[lbl, left] {Yes} (semi);
\draw[arr] (cost.east) -- node[lbl, above] {No} (manual);

\node[lbl] at (-2.5,1.0) {$<$100ms};
\node[lbl] at (0,0.1) {60s timeout};
\node[lbl] at (2.5,1.0) {Human required};
\end{tikzpicture}%
}
\caption{Governance decision flow. Critical failures trigger immediate auto-migration; non-critical failures follow cost-aware semi-automated or manual paths.}
