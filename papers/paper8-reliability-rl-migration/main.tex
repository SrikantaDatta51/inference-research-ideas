...
% --- Arrows: Infra -> Signal ---
\draw[arr, black!50] (3.6,1.8) -- (signals);

% --- Arrows: Signal -> RL Agents ---
\draw[arr] (signals.east) -- ++(1,0) |- (ppo.west);
\draw[arr] (signals.east) -- ++(1,0) |- (sac.west);
\draw[arr] (signals.east) -- ++(1,0) |- (cql.west);
...
% --- Feedback loop (routed below all boxes) ---
\draw[darr] (migrate.south) -- ++(0,-0.8) -- (-1.5,-0.8) -- (-1.5,3.6) -- (signals.west);
\node[font=\tiny\sffamily\itshape, text=black!40] at (6,-1.0) {Outcome feedback to RL training};
\end{tikzpicture}%
}
