...
\centering
\begin{tikzpicture}[
    mod/.style={rectangle,draw,rounded corners=4pt,minimum height=0.7cm,font=\scriptsize\sffamily,line width=0.5pt,minimum width=2.4cm,text=black},
    ...
    pipe/.style={rectangle,draw,rounded corners=3pt,minimum height=0.55cm,font=\scriptsize\sffamily,line width=0.4pt,minimum width=2cm,text=black},
    arr/.style={-{Stealth[length=2mm]},line width=0.5pt,black},
    darr/.style={-{Stealth[length=2mm]},line width=0.4pt,dashed,black},
]
% Voice Input Pipeline (left column, vertically stacked)
\node[pipe,fill=asr!8] (mic) at (-5.5,3.5) {Microphone};
\node[pipe,fill=asr!8] (vad) at (-5.5,2.5) {VAD (8\,ms)};
\node[pipe,fill=asr!8] (asr) at (-5.5,1.5) {Whisper-v3 ASR};
\node[pipe,fill=emo!6] (emo) at (-5.5,0.3) {Emotion Detector};

% Central LLM Backbone (light fill, black text)
\node[mod,fill=vox!6,minimum width=4.2cm,minimum height=1.2cm,font=\normalsize\sffamily\bfseries] (llm) at (0,2) {Llama-3.1-70B};
\node[font=\tiny\sffamily,text=black!50] at (0,1.15) {Shared Reasoning Backbone};

% LoRA Adapters (above backbone, evenly spaced)
\node[lora,fill=cust!6] (l1) at (-2.8,3.8) {billing-v3};
\node[lora,fill=cust!6] (l2) at (-0.6,3.8) {provision-v2};
\node[lora,fill=ops!6] (l3) at (1.6,3.8) {noc-triage-v4};
\node[lora,fill=ops!6] (l4) at (3.8,3.8) {incident-v2};
\node[font=\tiny\sffamily,text=black!50] at (-1.7,4.3) {CX Adapters};
\node[font=\tiny\sffamily,text=black!50] at (2.7,4.3) {Ops Adapters};

% Knowledge Graphs (below backbone, spaced apart)
\node[mod,fill=cust!5,minimum width=2.2cm] (ckg) at (-2.2,-0.1) {Customer KB};
\node[mod,fill=ops!5,minimum width=2.2cm] (okg) at (2.2,-0.1) {Operations KB};
\node[mod,fill=vox!4,minimum width=5cm] (shared) at (0,-1.1) {Shared Infra Knowledge Graph (73\% overlap)};

% Voice Output Pipeline (right column)
\node[pipe,fill=tts!6] (gen) at (5.5,2.8) {Response Gen};
\node[pipe,fill=tts!6] (tts) at (5.5,1.8) {XTTS-v2 TTS};
\node[pipe,fill=tts!6] (spk) at (5.5,0.8) {Speaker};

% RL Agent (bottom center)
\node[mod,fill=rl2!5,minimum width=3cm] (rl) at (0,-2.3) {\textbf{RL Policy ($\pi_\theta$)}};

% ── Input pipeline arrows (vertical, clean) ──
\draw[arr] (mic) -- (vad);
\draw[arr] (vad) -- (asr);
\draw[arr] (asr) -- (emo);

% ── ASR to LLM (L-shaped, avoids crossing) ──
\draw[arr] (asr.east) -- (-3.2,1.5) -- (-3.2,2.0) -- (llm.west);

% ── Emotion to RL (down then right) ──
\draw[arr] (emo.south) |- (rl.west);

% ── LoRA to LLM (short vertical drops, no overlap) ──
\draw[darr] (l1.south) -- ++(0,-0.65);
\draw[darr] (l2.south) -- ++(0,-0.65);
\draw[darr] (l3.south) -- ++(0,-0.65);
\draw[darr] (l4.south) -- ++(0,-0.65);

% ── LLM to output pipeline ──
\draw[arr] (llm.east) -- (gen.west);
\draw[arr] (gen) -- (tts);
\draw[arr] (tts) -- (spk);

% ── KBs to LLM (vertical, separated) ──
...
% ── RL to LLM (vertical, centered) ──
\draw[arr] (rl.north) -- ++(0,1.3);
\end{tikzpicture}
\caption{VoxOps unified architecture. A single LLM backbone serves both customer experience (CX) and network operations (Ops) via hot-swappable LoRA adapters. The shared infrastructure knowledge graph (73\% entity overlap) enables cross-domain context transfer. Emotion detection feeds the RL conversation policy. Voice pipeline achieves 188\,ms P50 end-to-end latency.}
\label{fig:arch}
\end{figure*}
