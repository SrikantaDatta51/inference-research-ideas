...
\centering
\begin{tikzpicture}[
    mod/.style={rectangle,draw,rounded corners=4pt,minimum height=0.7cm,font=\scriptsize\sffamily,line width=0.6pt,minimum width=2.4cm,text=black},
    lora/.style={rectangle,draw,rounded corners=2pt,minimum height=0.45cm,font=\tiny\sffamily\bfseries,line width=0.4pt,minimum width=1.8cm,text=black},
    pipe/.style={rectangle,draw,rounded corners=3pt,minimum height=0.55cm,font=\scriptsize\sffamily,line width=0.5pt,minimum width=2cm,text=black},
    arr/.style={-{Stealth[length=2mm]},line width=0.6pt,black},
    darr/.style={-{Stealth[length=2mm]},line width=0.5pt,dashed,black},
]
% Voice Input Pipeline (left column — purple tones)
\node[pipe,fill=asr!15] (mic) at (-5.5,3.5) {Microphone};
\node[pipe,fill=asr!18] (vad) at (-5.5,2.5) {VAD (8\,ms)};
\node[pipe,fill=asr!22] (asr) at (-5.5,1.5) {Whisper-v3 ASR};
\node[pipe,fill=emo!12] (emo) at (-5.5,0.3) {Emotion Detector};

% Central LLM Backbone (blue tint)
\node[mod,fill=vox!12,minimum width=4.2cm,minimum height=1.2cm,font=\normalsize\sffamily\bfseries] (llm) at (0,2) {Llama-3.1-70B};
\node[font=\tiny\sffamily,text=black!50] at (0,1.15) {Shared Reasoning Backbone};

% LoRA Adapters (green = CX, orange = Ops)
\node[lora,fill=cust!15] (l1) at (-2.8,3.8) {billing-v3};
\node[lora,fill=cust!15] (l2) at (-0.6,3.8) {provision-v2};
\node[lora,fill=ops!15] (l3) at (1.6,3.8) {noc-triage-v4};
\node[lora,fill=ops!15] (l4) at (3.8,3.8) {incident-v2};
\node[font=\tiny\sffamily,text=black!60] at (-1.7,4.3) {CX Adapters};
\node[font=\tiny\sffamily,text=black!60] at (2.7,4.3) {Ops Adapters};

% Knowledge Graphs (teal/green tones)
\node[mod,fill=cust!12,minimum width=2.2cm] (ckg) at (-2.2,-0.1) {Customer KB};
\node[mod,fill=ops!12,minimum width=2.2cm] (okg) at (2.2,-0.1) {Operations KB};
\node[mod,fill=vox!8,minimum width=5cm] (shared) at (0,-1.1) {Shared Infra Knowledge Graph (73\% overlap)};

% Voice Output Pipeline (right column — teal tones)
\node[pipe,fill=tts!15] (gen) at (5.5,2.8) {Response Gen};
\node[pipe,fill=tts!18] (ttsn) at (5.5,1.8) {XTTS-v2 TTS};
\node[pipe,fill=tts!12] (spk) at (5.5,0.8) {Speaker};

% RL Agent (bottom center — warm red)
\node[mod,fill=rl2!12,minimum width=3cm] (rl) at (0,-2.3) {\textbf{RL Policy ($\pi_\theta$)}};

% ── Input pipeline arrows ──
\draw[arr] (mic) -- (vad);
\draw[arr] (vad) -- (asr);
\draw[arr] (asr) -- (emo);
\draw[arr] (asr.east) -- (-3.2,1.5) -- (-3.2,2.0) -- (llm.west);
\draw[arr] (emo.south) |- (rl.west);

% ── LoRA to LLM ──
\draw[darr] (l1.south) -- ++(0,-0.65);
\draw[darr] (l2.south) -- ++(0,-0.65);
\draw[darr] (l3.south) -- ++(0,-0.65);
\draw[darr] (l4.south) -- ++(0,-0.65);

% ── LLM to output ──
\draw[arr] (llm.east) -- (gen.west);
\draw[arr] (gen) -- (ttsn);
\draw[arr] (ttsn) -- (spk);

% ── KBs to LLM ──
\draw[darr] (ckg.north) -- ++(0,0.7);
\draw[darr] (okg.north) -- ++(0,0.7);
\draw[arr] (ckg.south) -- (shared.north -| ckg.south);
\draw[arr] (okg.south) -- (shared.north -| okg.south);

% ── RL to LLM ──
\draw[arr] (rl.north) -- ++(0,1.3);
\end{tikzpicture}
\caption{VoxOps unified architecture. A single LLM backbone serves both CX and Ops via hot-swappable LoRA adapters. The shared knowledge graph (73\% overlap) enables cross-domain context. Emotion detection feeds the RL policy. Pipeline achieves 188\,ms P50 latency.}
\label{fig:arch}
\end{figure*}
...
\begin{tikzpicture}[
    ...
    box/.style={draw, rounded corners=3pt, minimum width=2.0cm, minimum height=0.6cm, font=\scriptsize, thick, text=black},
    arr/.style={-{Stealth[length=2mm]}, thick, black},
    darr/.style={-{Stealth[length=2mm]}, thick, black, dashed},
    lbl/.style={font=\tiny, text=black!60}
]
% === ROW 1: Audio Pipeline (top) ===
\node[box, fill=blue!15] (mic) at (0,0) {Microphone};
\node[box, fill=blue!18, right=0.8cm of mic] (vad) {Silero VAD};
\node[box, fill=blue!22, right=0.8cm of vad] (asr) {Whisper-v3};
\node[box, fill=purple!15, right=0.8cm of asr] (llm) {Llama-70B};
\node[box, fill=cyan!15, right=0.8cm of llm] (ttsn) {XTTS-v2};
\node[box, fill=cyan!12, right=0.8cm of ttsn] (spk) {Speaker};

% === ROW 2: Intent Pipeline (middle) ===
\node[box, fill=green!15, below=1.0cm of asr] (intent) {Intent Classifier};
\node[box, fill=green!18, right=0.8cm of intent] (prefill) {Spec. Prefill};

% === ROW 3: Emotion + Context (bottom) ===
\node[box, fill=red!12, below=1.0cm of intent, xshift=-1.5cm] (emo) {Emotion Det.};
\node[box, fill=orange!12, below=1.0cm of intent, xshift=1.5cm] (rl) {RL Policy};
\node[box, fill=yellow!15, below=1.0cm of prefill, xshift=1.5cm] (kg) {Knowledge Graph};

% === Arrows: Audio pipeline ===
\draw[arr] (mic) -- (vad);
\draw[arr] (vad) -- (asr);
\draw[arr] (asr) -- (llm);
\draw[arr] (llm) -- (ttsn);
...
% === Arrows: Intent branch (down from ASR, right to prefill, up to LLM) ===
\draw[arr] (asr.south) -- (intent.north);
\draw[arr] (intent) -- (prefill);
\draw[darr] (prefill.north) -- ++(0,0.55) -| (llm.south);

% === Arrows: Emotion branch ===
\draw[arr] (asr.south west) -- ++(0,-0.4) -| (emo.north);
\draw[arr] (emo) -- (rl);
\draw[darr] (rl.north) -- ++(0,0.5) -| (llm.south west);

% === Arrows: Knowledge Graph ===
\draw[darr] (kg.north) -- ++(0,0.5) -| (llm.south east);
\draw[arr] (asr.south east) -- ++(0,-0.4) -| (kg.north west);

% === Timing annotations ===
\node[lbl, above=0.05cm of vad] {8ms};
\node[lbl, above=0.05cm of asr] {80ms};
\node[lbl, above=0.05cm of llm] {60ms};
\node[lbl, above=0.05cm of ttsn] {30ms};
\node[lbl, above=0.05cm of intent] {20ms};
\end{tikzpicture}
...
\node[font=\tiny\sffamily\bfseries,anchor=east] at (-0.1,2.2) {Trad.};
\fill[gray!20] (0,1.8) rectangle (2.5,2.5) node[midway,font=\tiny,text=black]{ASR (125ms)};
\fill[gray!30] (2.5,1.8) rectangle (4.8,2.5) node[midway,font=\tiny,text=black]{LLM (115ms)};
\fill[gray!12] (4.8,1.8) rectangle (6.5,2.5) node[midway,font=\tiny,text=black]{TTS (85ms)};
\fill[gray!8] (6.5,1.8) rectangle (8,2.5) node[midway,font=\tiny,text=black]{Stream};
\draw[black,thick,dashed] (6.5,1.6) -- (6.5,2.7) node[above,font=\tiny,text=black]{325ms};

% VoxOps pipeline
\node[font=\tiny\sffamily\bfseries,anchor=east] at (-0.1,1) {VoxOps};
\fill[gray!20] (0,0.6) rectangle (1.8,1.3) node[midway,font=\tiny,text=black]{ASR};
\fill[gray!25] (1,0.6) rectangle (3,1.3) node[midway,font=\tiny,text=black]{Spec. Prefill};
\fill[gray!35] (1.8,0.6) rectangle (3.2,1.3);
\node[font=\tiny,text=black] at (2.5,0.95) {Decode};
\fill[gray!12] (2.8,0.6) rectangle (3.8,1.3) node[midway,font=\tiny,text=black]{TTS};
\draw[black,thick,dashed] (3.8,0.4) -- (3.8,1.5) node[above,font=\tiny,text=black]{188ms};
\end{tikzpicture}
...
\begin{equation}
R = w_1\text{CSAT} + w_2\text{FCR} - w_3\text{AHT} - w_4\text{Esc} + w_5\Delta\text{Emo} - w_6 C
\end{equation}
...
\begin{axis}[
    ...
    legend style={font=\tiny, at={(0.02,0.98)}, anchor=north west, draw=black!30},
    grid=major, grid style={gray!15},
    xticklabel style={font=\tiny}, yticklabel style={font=\tiny},
    xlabel style={font=\scriptsize}, ylabel style={font=\scriptsize},
]
\addplot[black, thick, mark=*, mark size=1.5pt] coordinates {(1,0.2)(2,0.25)(3,0.4)(4,0.55)(5,0.65)(6,0.72)(7,0.8)(8,0.85)(9,0.88)(10,0.92)};
\addplot[black, thick, dashed, mark=triangle*, mark size=2pt] coordinates {(1,0.2)(2,0.22)(3,0.28)(4,0.35)(5,0.42)(6,0.5)(7,0.55)(8,0.6)(9,0.63)(10,0.65)};
\addplot[black!50, thick, dotted, mark=square*, mark size=1.5pt] coordinates {(1,0.2)(2,0.18)(3,0.22)(4,0.3)(5,0.35)(6,0.4)(7,0.45)(8,0.5)(9,0.52)(10,0.55)};
