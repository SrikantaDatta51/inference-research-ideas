...
\textbf{Disaggregated Inference (GPU-to-GPU).} Splitwise~\cite{patel2024splitwise}, DistServe~\cite{zhong2024distserve}, NVIDIA Dynamo~\cite{nvidia2024b200}, and Mooncake~\cite{qin2024mooncake} all separate prefill from decode but route \emph{both phases to GPU hardware}. Splitwise demonstrated 1.4$\times$ throughput via phase separation; Dynamo adds KV-cache routing but is locked to NVIDIA hardware. \textbf{Critical distinction}: all prior disaggregated systems disaggregate \emph{which GPU}, not \emph{which hardware class}. Our architecture is the first to route decode to CPU clusters (\$3.20/hr vs. \$31.20/hr for equivalent memory-bound throughput).

\textbf{CPU Inference and Serving.} llama.cpp~\cite{gerganov2024llamacpp} pioneered CPU inference with SIMD kernels. PowerInfer~\cite{song2024powerinfer} and FlexGen~\cite{sheng2023flexgen} optimize single-node CPU-GPU execution. vLLM~\cite{kwon2023vllm} provides PagedAttention; SGLang~\cite{zheng2024sglang} adds RadixAttention; MemoryAlloy~\cite{crusoe2025memoryalloy} achieves 9.9$\times$ TTFT via cluster KV-cache sharing. These are single-node or GPU-only optimizations. Our work integrates them into a \emph{fleet-level heterogeneous} architecture with CXL memory pooling and cross-architecture KV-cache transfer.

\textbf{CXL and Speculative Decoding.} TPP~\cite{maruf2023cxl} and Sun et al.~\cite{sun2024cxl} evaluate CXL memory; we provide the \emph{first CXL~3.1 evaluation for KV-cache in CPU decode}. EAGLE-2~\cite{li2024eagle2} achieves 2.1--3.4 tokens/step on GPUs; our CPU-native implementation dedicates idle cores to draft execution at \emph{zero marginal cost}, achieving 2.8$\times$ speedup. GPTQ~\cite{frantar2023gptq} and AWQ~\cite{lin2024awq} enable INT4; our GGUF SIMD-tiled layouts are 15--25\% faster on CPUs. We benchmark across NVIDIA B200~\cite{nvidia2024b200}, AMD MI300X~\cite{amd2024mi300x}, Intel Gaudi~3~\cite{intel2024gaudi3}, ARM Graviton4, and DeepSeek-V3~\cite{deepseek2024v3} MoE.
...
\textbf{Key novelties not present in any prior system}: (1)~\textbf{GPU$\to$CPU phase routing}---the first to route decode to CPUs at fleet scale; (2)~\textbf{Cross-architecture KV-cache transfer} via CXL~3.1 (GPU HBM$\to$CPU DDR5, zero-copy); (3)~\textbf{Zero-marginal-cost speculative decoding}---idle CPU cores run the EAGLE-2 draft model for free, improving acceptance by 33\%; (4)~\textbf{Vendor-agnostic disaggregation} across NVIDIA/AMD/Intel prefill and x86/ARM decode; (5)~\textbf{MoE single-server CPU decode}---DeepSeek-V3 (685B) at \$3.20/hr via expert sparsity.

\textbf{Limitations}: CPU decode trails GPUs at high batch sizes ($>$32); speculative acceptance varies (2.1--3.4 tokens/draft); CXL~3.1 hardware limited in 2026.

\textbf{Decision framework}: CPU decode for output $<$512 tokens, MoE, batch, power-constrained. GPU-only for ultra-low-latency ($<$5\,ms TPOT), high batch ($>$64). Hybrid for RAG, multi-model, cost-sensitive ($>$1M tok/hr). \textbf{The industry is not NVIDIA-only}: AMD MI300X at \$1.50/GPU/hr and ARM Graviton4 at \$2.42/hr achieve competitive cost-per-token.
