...
\caption{Multi-vendor CPU-centric disaggregated inference architecture. Four vendor stacks operate in parallel: \textbf{NVIDIA} (B200/H100 via NVLink~6.0 for compute-bound prefill with FlashAttention-3 and TensorRT-LLM); \textbf{AMD} (MI300X with 192\,GB HBM3 via Infinity Fabric for prefill and large-batch decode with ROCm/vLLM); \textbf{Intel} (Gaudi~3 for cost-optimized prefill via Ethernet RoCE, Granite Rapids AMX for CPU decode with SynapseAI/oneAPI); \textbf{ARM} (Graviton4 SVE2 for low-power decode at lowest \$/token). All stacks share a unified CXL~3.1 memory pool and MemoryAlloy KV-cache fabric. Storage tiers: NVMe Gen5, NVMe-oF, S3/GCS. Aggregate: 47$\times$ cost reduction to \$0.059/M tokens.}
\label{fig:arch}
\end{figure*}
...
\midrule
\multicolumn{6}{c}{\textit{GPU/Accelerator Stacks (Prefill + Decode)}} \\
\midrule
NVIDIA & 8$\times$H100 SXM & 26.8\,TB/s & \$49.28 & 544 & 640\,GB \\
NVIDIA & 8$\times$H200 SXM & 35.2\,TB/s & \$50.48 & 697 & 1.1\,TB \\
AMD & 8$\times$MI300X & 42.4\,TB/s & \$12.00 & 3533 & 1.5\,TB \\
Intel & 8$\times$Gaudi~3 & 29.6\,TB/s & \$24.00 & 1233 & 1.0\,TB \\
\midrule
\multicolumn{6}{c}{\textit{CPU Stacks (Decode Only)}} \\
\midrule
Intel & 2$\times$Granite Rapids & 410\,GB/s & \$4.20 & 98 & 2\,TB \\
...
\caption{Cross-vendor disaggregated inference pairing matrix. Each cell shows \$/M output tokens and performance rating for a prefill$\to$decode hardware combination. Green cells indicate $>$5$\times$ cost savings vs. GPU-only baseline; yellow cells 3--5$\times$ savings. Optimal pairing: NVIDIA B200 prefill + ARM Graviton4 decode (\$0.059/M, 47$\times$ reduction). Best all-AMD: MI300X + EPYC (\$0.072/M). Best all-Intel: Gaudi~3 + Granite Rapids (\$0.079/M).}
...
TCP+INT3 (cross-cloud) & 25\,Gb/s & 78\,ms & 0.24\,GB & Yes \\
Delta INT3 (agentic) & 100\,Gb/s & 1.2\,ms & 0.015\,GB & Optimal \\
\bottomrule
\end{tabular}
\end{table}
