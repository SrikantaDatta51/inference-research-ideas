...
\author{
\IEEEauthorblockN{Srikanta Datta Tumkur\IEEEauthorrefmark{1}, Jay Iyer\IEEEauthorrefmark{2}, Anshu Bansal\IEEEauthorrefmark{1}, Mehar Simhadri\IEEEauthorrefmark{2}}
...
\textbf{Disaggregated Inference (GPU-to-GPU).} Splitwise~\cite{patel2024splitwise} and DistServe~\cite{zhong2024distserve} separate prefill and decode within a single cloud, using \emph{GPU nodes for both phases}. Splitwise demonstrated 1.4$\times$ throughput improvement via phase disaggregation; DistServe extended this with goodput-optimal placement. NVIDIA Dynamo~\cite{nvidia2024b200} provides a GPU-centric orchestration layer with KV-cache routing and worker scaling, but remains locked to NVIDIA hardware and does not consider heterogeneous CPU decode. Mooncake~\cite{qin2024mooncake} provides KV-cache-centric disaggregated serving but again within a GPU-only fleet.

\textbf{Critical distinction from prior disaggregated systems}: All existing disaggregated serving frameworks (Splitwise, DistServe, Dynamo, Mooncake) route \emph{both} prefill and decode to GPU hardware, merely separating them onto different GPU nodes. Our architecture is the first to route decode to CPU clusters, which represents a fundamentally different economic proposition: CPU decode at \$3.20/hr vs. GPU decode at \$31.20/hr for equivalent throughput on memory-bound workloads.

\textbf{CPU Inference Engines.} llama.cpp~\cite{gerganov2024llamacpp} pioneered CPU inference with SIMD kernels and GGUF quantization. PowerInfer~\cite{song2024powerinfer} introduced activation-locality-aware hybrid execution. FlexGen~\cite{sheng2023flexgen} optimized throughput via CPU-GPU-disk offloading. Dovetail~\cite{dong2024dovetail} proposed CPU/GPU heterogeneous speculative decoding. These are \emph{single-node} optimizations. Our work integrates them into a \emph{fleet-level} production architecture with CXL memory pooling and cross-node KV-cache fabrics---a qualitatively different system.

\textbf{Serving Systems.} vLLM~\cite{kwon2023vllm} introduced PagedAttention for KV-cache management. SGLang~\cite{zheng2024sglang} provides RadixAttention for prefix sharing. Crusoe MemoryAlloy~\cite{crusoe2025memoryalloy} demonstrated 9.9$\times$ TTFT improvement through cluster-wide KV-cache sharing. Our work extends prefix sharing to heterogeneous CPU/GPU fleets where KV-cache must be transferred across architectures (GPU$\to$CPU via CXL/RDMA).

\textbf{CXL Memory.} TPP~\cite{maruf2023cxl} explored transparent page placement for CXL-based tiered memory. Sun et al.~\cite{sun2024cxl} demystified CXL memory performance. Our work provides the \emph{first evaluation of CXL~3.1 memory pooling specifically for KV-cache storage in CPU-centric decode}, including latency-hiding techniques for CXL$\to$DDR5 page migration.

\textbf{Speculative Decoding.} EAGLE-2~\cite{li2024eagle2} achieves 2.1--3.4 accepted tokens per step on GPUs. Medusa~\cite{cai2024medusa} adds parallel prediction heads. Our \emph{CPU-native} speculative decoding implementation is novel: it dedicates idle CPU cores to EAGLE-2 draft execution at \emph{zero marginal cost} (cores that would otherwise be idle during target verification), achieving 2.8$\times$ speedup vs.~2.1$\times$ on GPU where draft execution competes for the same compute.

\textbf{Quantization.} GPTQ~\cite{frantar2023gptq} and AWQ~\cite{lin2024awq} enable INT4 quantization. Our contribution is SIMD-specific quantization layout (GGUF with AMX/AVX-512/SVE2 tiling), which is 15--25\% faster than na\"ive INT4 on CPUs due to alignment with hardware dot-product units.

\textbf{Hardware.} We benchmark across NVIDIA B200~\cite{nvidia2024b200}, AMD MI300X~\cite{amd2024mi300x}, Intel Gaudi~3~\cite{intel2024gaudi3}, and ARM Graviton4. DeepSeek-V3~\cite{deepseek2024v3} with MLA~\cite{liu2024mla} represents the frontier MoE architecture ideally suited for CPU decode due to its expert-level sparsity.
...
\subsection{Novelty vs.\ Existing Disaggregated Frameworks}

Table~\ref{tab:novelty} summarizes the critical differences between our approach and existing systems:

\begin{table}[!htbp]
\caption{Differentiation from Existing Disaggregated Serving}
\label{tab:novelty}
\centering\scriptsize
\resizebox{\linewidth}{!}{
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Capability} & \textbf{Ours} & \textbf{Dynamo} & \textbf{Splitwise} & \textbf{Mooncake} & \textbf{DistServe} \\
\midrule
CPU decode & \checkmark & --- & --- & --- & --- \\
Cross-vendor (Intel/AMD/ARM) & \checkmark & --- & --- & --- & --- \\
CXL 3.1 KV-cache pooling & \checkmark & --- & --- & --- & --- \\
CPU speculative decode & \checkmark & --- & --- & --- & --- \\
Cluster KV-cache fabric & \checkmark & Partial & --- & \checkmark & --- \\
Workload-aware routing & \checkmark & Partial & --- & --- & \checkmark \\
MoE single-server decode & \checkmark & --- & --- & --- & --- \\
\$/M tok (70B, best) & \$0.059 & \$2.80 & \$1.90 & \$1.40 & \$1.60 \\
\bottomrule
\end{tabular}
}
\end{table}

\textbf{Key novelties not present in any prior system}:
\begin{enumerate}
\item \textbf{GPU$\to$CPU phase routing}: We are the first to route decode to CPUs at fleet scale. Dynamo, Splitwise, DistServe, and Mooncake all use GPUs for both phases---they disaggregate \emph{which GPU}, not \emph{which hardware class}.
\item \textbf{Cross-architecture KV-cache transfer}: Our CXL~3.1 fabric enables zero-copy KV-cache migration from GPU HBM to CPU DDR5, a capability no existing system supports.
\item \textbf{Zero-marginal-cost speculative decoding}: On GPUs, draft model execution competes for the same compute; on CPUs, idle cores run the draft model \emph{for free}, improving acceptance rates by 33\%.
\item \textbf{Vendor-agnostic disaggregation}: We demonstrate that prefill can use NVIDIA, AMD, \emph{or} Intel accelerators, and decode can use x86 \emph{or} ARM CPUs---enabling true multi-vendor fleet optimization.
\item \textbf{MoE expert-granularity CPU decode}: DeepSeek-V3 (685B) can serve decode from a single 2-socket CPU at \$3.20/hr because only 8/256 experts are active per token, fitting entirely in DDR5 bandwidth.
\end{enumerate}

\subsection{Limitations}

CPU decode trails GPUs at high batch sizes ($>$32); speculative decoding acceptance rates vary (2.1--3.4 tokens/draft); CXL 3.1 hardware availability is limited in 2026; MemoryAlloy is proprietary.
