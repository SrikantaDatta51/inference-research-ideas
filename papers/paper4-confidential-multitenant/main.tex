...
\author{
\IEEEauthorblockN{Srikanta Datta Tumkur\IEEEauthorrefmark{1}, Jay Iyer\IEEEauthorrefmark{2}, Mehar Simhadri\IEEEauthorrefmark{2}, Anshu Bansal\IEEEauthorrefmark{1}}
...
\caption{AI Security Compliance Query Graph. Central \texttt{LLM Model} node connects to six domains: training data provenance (with bias scores), SBOM attestation (SLSA~L3, 0~CVEs via hardened base images), deployment context (sovereignty, GDPR), runtime risks (cross-tenant leakage, prompt injection), fine-tune lineage (parent-child bias delta tracking), and access control (per-tenant RBAC). Bottom: example Cypher query for identifying bias-risk models.}
\label{fig:graph}
\end{figure}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{fig6_provenance.png}
\caption{Model provenance and bias attestation pipeline. Six stages from training data (with dataset hashes, license audit, bias pre-scan) through base model training, fine-tuning with parent hash verification, bias gate with three-tier escalation ($\Delta < 0.5$: Green, $0.5$--$1.0$: Yellow, $\Delta \geq 1.0$: Red/blocked), TEE deployment with attestation, to production inference with continuous drift detection. Immutable audit trail spans all stages.}
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{fig4_throughput.png}
\caption{Throughput comparison across confidential computing configurations. GPU baseline (123.5~tok/s) drops by only 7\% with NVIDIA CC and 8\% with full governance. CPU TDX decode operates at lower absolute throughput but with only 2.1--5.2\% CC overhead. The $<$8\% bracket highlights that confidential+governed GPU inference remains within acceptable production overhead.}
\label{fig:throughput}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{fig5_tco.png}
\caption{Per-tenant cost comparison. Multi-tenant sharing with confidential computing achieves 4$\times$ savings (GPU: \$6.16 $\rightarrow$ \$1.54/tenant) and 6$\times$ savings (CPU TDX: \$4.20 $\rightarrow$ \$1.05/tenant). Orange line shows throughput overhead remains below 15\%. Net effect: 75\% cost reduction per tenant with full security hardening.}
\label{fig:tco}
\end{figure}

\begin{figure}[!htbp]
\centering
\includegraphics[width=\linewidth]{fig7_waterfall.png}
\caption{Waterfall analysis of cumulative throughput impact per security hardening layer (H100 SXM, Llama-3.1-70B). TEE memory encryption ($-4.1\%$) and GPU CC mode ($-2.8\%$) dominate overhead. Governance ($-1.1\%$), bias monitoring ($-0.6\%$), and audit logging ($-0.4\%$) contribute minimal impact. Total: 15.8\% from 123.5 to 104.0~tok/s for a fully hardened, governed, multi-tenant deployment.}
\label{fig:waterfall}
\end{figure}
